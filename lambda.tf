...
locals {
  lambda_functions = {
    lambda_stop_instances = {
      handler     = "stop_instances_lambda.lambda_handler"
      source_file = "stop_instances_lambda.py"
    }
    lambda_start_instances = {
      handler     = "start_instances_lambda.lambda_handler"
      source_file = "start_instance_lambda.py"
    }
  }
}
...
resource "aws_lambda_permission" "allow_start_event" {
  statement_id  = "AllowExecutionFromEventStart"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.lambda_start_instances.function_name
  principal     = "events.amazonaws.com"
  source_arn    = aws_cloudwatch_event_rule.start_instancea.arn
}
...
resource "aws_lambda_function" "lambda_start_instances" {
  for_each      = local.lambda_functions
  filename      = data.archive_file.lambda_stop[each.key].output_path
  function_name = each.key
  role          = aws_iam_role.lambda_startstop_instances_role.arn
  handler       = each.value.handler
  runtime       = "python3.9"
